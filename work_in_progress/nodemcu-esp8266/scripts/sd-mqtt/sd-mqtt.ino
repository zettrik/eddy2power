...
const int sleepTime = 300; // sleep time between readings in seconds
...
ADC_MODE(ADC_VCC); //read ESP voltage instead of analog_in A0
...
void write_sd() {
  ...
  if (dataFile) {
    analog_in = multiple_read(5);
    snprintf(mqtt_message, 100, "node1 to sd: %04d,%ld", analog_in, now);
    mqtt_pub("nodes", mqtt_message);
